		...
		xml.cXML(:payloadID => $shipnotices[$j]['payload_id'], :timestamp => $shipnotices[$j]['datetime']) {
			...
			xml.Request {
				xml.ShipNoticeRequest {
					xml.ShipNoticeHeader(:noticeDate => $shipnotices[$j]['datetime'], :operation => 'new', :deliveryDate => $shipnotices[$j]['delivery_date'], :shipmentDate => $shipnotices[$j]['shipping_date'], :orderID => $shipnotices[$j]['order_number'], :shipmentID => $shipnotices[$j]['delivery_id']) {
