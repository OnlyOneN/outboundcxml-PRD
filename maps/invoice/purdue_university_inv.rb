builder = Nokogiri::XML::Builder.new(:encoding => 'UTF-8') do |xml|
	xml.doc.create_internal_subset('cXML', nil, "http://xml.cXML.org/schemas/cXML/1.2.024/InvoiceDetail.dtd")
	xml.cXML(:payloadID => $invoices[$j]['payload_id'], :timestamp => $datetime) {
		...
		xml.Request(:deploymentMode => $deploymentmode) {
			xml.InvoiceDetailRequest {
				xml.InvoiceDetailRequestHeader(:invoiceDate => $invoices[$j]['invoice_date'], :invoiceID => $invoices[$j]['invoice_id'], :invoiceOrigin => 'supplier', :operation => 'new', :purpose => 'standard') {
					xml.InvoiceDetailHeaderIndicator
					xml.InvoiceDetailLineIndicator
					xml.InvoicePartner {
						xml.Contact(:addressID => '0000004678-0002001903', :role => 'remitTo') {
					...
					xml.InvoicePartner {
						xml.Contact(:role => 'from') {
							xml.Name('New England Biolabs', :"xml:lang" => 'en')
							xml.PostalAddress {
								xml.Street '240 County Road'
								xml.City 'Ipswich'
								xml.State 'MA'
								xml.PostalCode '01938-2723'
								xml.Country('United States', :isoCountryCode => 'US')
							}
						}
					}
					...
					xml.InvoiceDetailShipping(:shippingDate => $invoices[$j]['shipping_date']) {
