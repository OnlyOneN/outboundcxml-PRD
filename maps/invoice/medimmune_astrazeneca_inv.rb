		...
		xml.Request(:deploymentMode => $deploymentmode) {
			xml.InvoiceDetailRequest {
				xml.InvoiceDetailRequestHeader(:invoiceDate => $invoices[$j]['invoice_date'], :invoiceID => $invoices[$j]['invoice_id'], :invoiceOrigin => 'supplier', :operation => 'new', :purpose => 'standard') {
					...
					xml.InvoicePartner {
						xml.Contact(:addressID => '0010000854_0030000800', :role => 'remitTo') {
					...
					xml.InvoiceDetailShipping(:shippingDate => $invoices[$j][38]) {
