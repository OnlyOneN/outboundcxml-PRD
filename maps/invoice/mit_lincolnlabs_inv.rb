builder = Nokogiri::XML::Builder.new(:encoding => 'UTF-8') do |xml|
	...
	xml.cXML(:payloadID => $invoices[$j]['payload_id'], :timestamp => $datetime) {
		case $invoices[$j]['account_number']
		 	when '0001000539_0000200539'
		 		fromIdentity = '0000159828'
		 		toIdentity = 'MIT'
		 	else
		 		fromIdentity = '0001001737'
		 		toIdentity = 'Lincoln Lab'
		 	end
		xml.Header {
			xml.From {				
				xml.Credential(:domain => $invoices[$j]['from_domain']) {
					xml.Identity fromIdentity
				}						
			}		
			xml.To {
				xml.Credential(:domain => $invoices[$j]['to_domain']) {
					xml.Identity toIdentity
				}											
			}
			xml.Sender {
				...
				xml.UserAgent 'New England Biolabs, Inc.'											
			}									
		}
		xml.Request(:deploymentMode => $deploymentmode) {
			xml.InvoiceDetailRequest {
				xml.InvoiceDetailRequestHeader(:invoiceDate => $invoices[$j]['invoice_date'], :invoiceID => $invoices[$j]['invoice_id'], :invoiceOrigin => 'supplier', :operation => 'new', :purpose => 'standard') {
					...
					xml.InvoicePartner {
						xml.Contact(:addressID => '', :role => 'remitTo') {
