		...
		xml.cXML(:payloadID => $confirmations[$j]['payload_id'], :timestamp => $confirmations[$j]['datetime']) {
			...
			xml.Request(:deploymentMode => $deploymentmode) {
				xml.ConfirmationRequest {
					...
					i = 0
					$detailLines.each do
						$nextday = (Time.parse($detailLines[i]['shipping_date']) + 1 * 86400).strftime("%FT%T%:z") # 24 * 60 * 60
					 	if ($detailLines[i]['confirmed_quantity'].to_i == 0)
					 		xml.ConfirmationItem(:lineNumber => $detailLines[i]['customer_line_number'].to_i, :quantity => $detailLines[i]['ordered_quantity'].to_i) {
					 			...
					 			xml.ConfirmationStatus(:type => 'backordered', :quantity => $detailLines[i]['ordered_quantity'].to_i, :shipmentDate => $detailLines[i]['shipping_date'], :deliveryDate => $nextday) {
					 				xml.UnitOfMeasure 'EA'
					 			}
					 		}	
					 	else
					 		if ($detailLines[i]['ordered_quantity'].to_i - $detailLines[i]['confirmed_quantity'].to_i > 0) 
					 			...
					 			xml.ConfirmationItem(:lineNumber => $detailLines[i]['customer_line_number'].to_i, :quantity => $detailLines[i]['ordered_quantity'].to_i) {
					 				xml.UnitOfMeasure 'EA'	
					 				xml.ConfirmationStatus(:type => 'backordered', :quantity => $detailLines[i]['ordered_quantity'].to_i - $detailLines[i]['confirmed_quantity'].to_i, :shipmentDate => $detailLines[i]['shipping_date'], :deliveryDate => $nextday) {
					 					xml.UnitOfMeasure 'EA'
					 				}
					 				xml.ConfirmationStatus(:type => 'accept', :quantity => $detailLines[i]['confirmed_quantity'].to_i, :shipmentDate => $detailLines[i]['shipping_date'], :deliveryDate => $nextday) {
					 					xml.UnitOfMeasure 'EA'
					 				}
					 			}	
					 			else
